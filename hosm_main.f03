...
subroutine init_potential(phi, x, y, nx, ny)
    ...
    omega = dsqrt(g * dtanh(H))
    sigma = 1.0_C_DOUBLE ! H inf <-> deep water
    phi = A * dsin(x) * (-omega / sigma)
...
program fftw_test
    ...
    real(C_DOUBLE), dimension(Nx, Ny) :: x, y, eta, phi, in
...
include 'fftw3.f03'
    ...
    forall(i = 1: Nx / 2, j = 1: Ny) kx(i, j) = 2.0_C_DOUBLE * pi * (i - 1) / Lx
    kx(Nx / 2 + 1, : ) = 0.0_C_DOUBLE
    forall(i = 1: Nx / 2 + 1, j = 1: Ny / 2) ky(i, j) = 2.0_C_DOUBLE * pi * (j - 1) / Ly
    forall(i = 1: Nx / 2 + 1, j = Ny / 2 + 1: Ny) ky(i, j) = 2.0_C_DOUBLE * pi * (j - Ny - 1) / Ly

    forall(i = 1: Nx / 2 + 1, j = 1: Ny)
        k_mod(i, j) = dsqrt(kx(i, j) ** 2 + ky(i, j) ** 2)
        sigma(i, j) = dtanh(k_mod(i, j) * H)
    end forall
